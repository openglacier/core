...
use super::{ coerce_value_pair_to_numbers, CoercedNumber, CoercionFailure, CoercionPolicy, Number, Value, };
...
pub fn less_than_or_equal( left: &Value, right: &Value, policy: CoercionPolicy, ) -> CompareResult<bool> {
...
pub fn greater_than_or_equal( left: &Value, right: &Value, policy: CoercionPolicy, ) -> CompareResult<bool> {
    compare(left, right, policy).map(Comparison::is_greater_or_equal)
}

pub fn compare_numbers( left: Number, right: Number, policy: CoercionPolicy, ) -> CompareResult<Comparison> {
    let left = Value::Number(left);
    let right = Value::Number(right);
    compare_numeric_values(&left, &right, policy)
}

fn compare_numeric_values( left: &Value, right: &Value, policy: CoercionPolicy, ) -> CompareResult<Comparison> {
