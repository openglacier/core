...
use crate::helpers::{decode_base64, encode_base64, u128_to_u64_saturating, unix_time_millis, hex};
...
pub enum Principal {
    #[default]
    Anonymous,
    Identity {identity_id: String, device_id: String, },
}
...
impl ConnectionAuth {
    ...
    pub fn begin(&mut self, credential: &DeviceCredential, ttl: Duration) -> Result<AuthChallenge, AuthError> {
        if !credential.active {
            return Err(AuthError::DeviceMismatch);
        }
        if credential.algorithm != "ed25519" {
    ...
    /// Starts a stateless enrollment challenge using Ed25519
    pub fn begin_enrollment(&mut self, identity_id: String, identity_public_key: String, device_id: String, device_public_key: String, ttl: Duration) -> Result<AuthChallenge, AuthError> {
    ...
    pub fn complete_enrollment(&mut self, challenge_id: &str, signature: &str) -> Result<EnrollmentIdentity, AuthError> {
        let pending = self
            .enrollment
            .take()
    ...
    pub fn complete(&mut self, challenge_id: &str, signature: &str, credential: &DeviceCredential) -> Result<Principal, AuthError> {
...
#[cfg(not(feature = "identity"))]
pub fn verify_ed25519(_public_key: &str, _signature: &str, _message: &[u8]) -> Result<(), AuthError> {
    Err(AuthError::UnsupportedAlgorithm("ed25519 support is not compiled into this build".to_owned()))
}
