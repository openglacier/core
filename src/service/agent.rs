...
use crate::{
    ...
    helpers::{unix_time_nanos, JsonFields},
        ...
        for partial in &self.partials {
            match partial.str_field("type") {
                ...
                Some("toolCall") => {
                    ...
                    let name = partial.str_field("name")
                        .filter(|value| !value.trim().is_empty())
                        .ok_or_else(|| {
                            AgentError::InvalidToolCall(
...
fn describe_collection( invoker: &mut dyn AgentCapabilityInvoker, arguments: &JsonValue, ) -> Result<JsonValue, AgentError> {
    let collection = arguments.str_field("collection")
        .ok_or_else(|| {
            AgentError::InvalidToolCall(
...
fn extract_count(response: &AgentCapabilityResponse) -> Option<u64> {
    response_documents(response)
        .into_iter()
        .find_map(|document| document.u64_field("count"))
}
...
fn tool_operation( name: &str, arguments: JsonValue, ) -> Result<(&'static str, JsonValue), AgentError> {
    match name {
        ...
        "database.query" => {
            let query = arguments.str_field("query")
                .ok_or_else(|| {
                    AgentError::InvalidToolCall(
...
mod tests {
    ...
    #[test] fn direct_answer_needs_only_one_llm_call() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![vec![json!({"type":"token","text":"Hello"})]], llm_requests: Vec::new(), }; let mut events = Vec::new(); let stats = service .run(&request(), &mut invoker, |event| { events.push(event); true }) .unwrap(); assert_eq!(stats.llm_calls, 1); assert_eq!(stats.tool_calls, 0); assert_eq!(invoker.calls, vec!["llm.generate"]); assert!(events.iter().any(|event| { event.str_field("type") == Some("message") && event.str_field("content") == Some("Hello") })); }
    #[test] fn native_tool_call_round_trips_as_assistant_and_tool_messages() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | limit 1"} })], vec![json!({"type":"token","text":"Ada"})], ], llm_requests: Vec::new(), }; let stats = service.run(&request(), &mut invoker, |_| true).unwrap(); assert_eq!(stats.llm_calls, 2); assert_eq!(stats.tool_calls, 1); assert_eq!( invoker.calls, vec!["llm.generate", "query.execute", "llm.generate"] ); let first = &invoker.llm_requests[0]; assert_eq!( first.array_field("tools").map(Vec::len), Some(4) ); let first_cache_key = first.str_field("cacheKey").unwrap(); assert!(first_cache_key.starts_with("agent-")); let second = &invoker.llm_requests[1]; assert_eq!( second.str_field("cacheKey"), Some(first_cache_key) ); assert_eq!( second.str_field("toolChoice"), Some("auto") ); assert_eq!( second.array_field("tools").map(Vec::len), Some(2) ); assert_eq!( second.u64_field("maxTokens"), Some(u64::from(DEFAULT_LLM_MAX_TOKENS)) ); let second_messages = second.array_field("messages") .unwrap(); let assistant = &second_messages[second_messages.len() - 2]; assert_eq!( assistant.str_field("role"), Some("assistant") ); assert_eq!( assistant.array_field("toolCalls") .and_then(|calls| calls.first()) .and_then(|call| call.get("id")) .and_then(JsonValue::as_str), Some("call-1") ); let tool = &second_messages[second_messages.len() - 1]; assert_eq!(tool.str_field("role"), Some("tool")); assert_eq!( tool.str_field("toolCallId"), Some("call-1") ); }
    #[test] fn database_query_uses_gateway_canonical_read_only_payload() { let (operation, data) = tool_operation("database.query", json!({"query":"on people | limit 1"})).unwrap(); assert_eq!(operation, "query.execute"); assert_eq!(data.str_field("query"), Some("on people | limit 1")); assert_eq!(data.bool_field("readOnly"), Some(true)); assert!(data.get("read_only").is_none()); }
    ...
    #[test] fn invalid_query_is_returned_to_model_for_retry() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"{'select': ['*'], 'from': 'people'}"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on people | limit 20"} })], vec![json!({"type":"token","text":"Ada"})], ], llm_requests: Vec::new(), }; let stats = service.run(&request(), &mut invoker, |_| true).unwrap(); assert_eq!(stats.llm_calls, 3); assert_eq!(stats.tool_calls, 2); assert_eq!( invoker.calls, vec!["llm.generate", "llm.generate", "query.execute", "llm.generate"] ); let retry_messages = invoker.llm_requests[1].array_field("messages") .expect("retry messages"); let tool_message = retry_messages.last().expect("tool error message"); assert_eq!(tool_message.str_field("role"), Some("tool")); let content = tool_message.str_field("content") .expect("tool error content"); assert!(content.contains("native OpenGlacier pipeline DSL")); assert!(content.contains("database.describe")); }
    #[test] fn summary_request_requires_collection_description_first() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Fais moi une synthèse de la table data_people".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.describe", "arguments":{"collection":"data_people"} })]], llm_requests: Vec::new(), }; let result = service.run(&input, &mut invoker, |event| { event.str_field("type") != Some("toolCall") }); assert!(matches!(result, Err(AgentError::Cancelled))); assert_eq!( invoker.llm_requests[0].str_field("toolChoice"), Some("required") ); assert_eq!( invoker.llm_requests[0].array_field("tools") .and_then(|tools| tools.first()) .and_then(|tool| tool.get("name")) .and_then(JsonValue::as_str), Some("database.describe") ); }
    ...
    #[test] fn schema_only_request_describes_then_synthesizes() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Quels champs sont présents dans la table data_people ?".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.describe", "arguments":{"collection":"data_people"} })], vec![json!({"type":"token","text":"name, country, score"})], ], llm_requests: Vec::new(), }; service.run(&input, &mut invoker, |_| true).unwrap(); assert_eq!(invoker.llm_requests.len(), 2); assert_eq!(invoker.llm_requests[0].str_field("toolChoice"), Some("required")); assert_eq!(invoker.llm_requests[1].str_field("toolChoice"), Some("none")); assert_eq!(invoker.llm_requests[1].array_field("tools").map(Vec::len), Some(0)); }
    #[test] fn summary_requires_one_successful_query_before_auto_answer() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Fais moi une synthèse de la table data_people".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.describe", "arguments":{"collection":"data_people"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on data_people | group country | sort count desc | limit 20"} })], vec![json!({"type":"token","text":"Summary"})], ], llm_requests: Vec::new(), }; service.run(&input, &mut invoker, |_| true).unwrap(); assert_eq!(invoker.llm_requests.len(), 3); assert_eq!(invoker.llm_requests[1].str_field("toolChoice"), Some("required")); let required_names = invoker.llm_requests[1].array_field("tools").unwrap().iter().filter_map(|tool| tool.str_field("name")).collect::<Vec<_>>(); assert_eq!(required_names, vec!["database.query"]); assert_eq!(invoker.llm_requests[2].str_field("toolChoice"), Some("auto")); let auto_names = invoker.llm_requests[2].array_field("tools").unwrap().iter().filter_map(|tool| tool.str_field("name")).collect::<Vec<_>>(); assert_eq!(auto_names, vec!["database.query"]); }
    #[test] fn detailed_statistics_require_two_successful_queries_before_auto_answer() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Quelles données sont présentes dans data_people ? Avec valeurs et statistiques ?".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.describe", "arguments":{"collection":"data_people"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on data_people | group country | sort count desc | limit 20"} })], vec![json!({ "type":"toolCall", "id":"call-3", "name":"database.query", "arguments":{"query":"on data_people | distinct name | limit 20"} })], vec![json!({"type":"token","text":"Detailed statistics"})], ], llm_requests: Vec::new(), }; service.run(&input, &mut invoker, |_| true).unwrap(); assert_eq!(invoker.llm_requests.len(), 4); for request in &invoker.llm_requests[1..=2] { assert_eq!(request.str_field("toolChoice"), Some("required")); let names = request.array_field("tools").unwrap().iter().filter_map(|tool| tool.str_field("name")).collect::<Vec<_>>(); assert_eq!(names, vec!["database.query"]); } assert_eq!(invoker.llm_requests[3].str_field("toolChoice"), Some("auto")); let final_names = invoker.llm_requests[3].array_field("tools").unwrap().iter().filter_map(|tool| tool.str_field("name")).collect::<Vec<_>>(); assert_eq!(final_names, vec!["database.query"]); }
    #[test] fn prose_during_required_query_phase_is_not_accepted_and_is_retried() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Fais moi une synthèse de la table data_people".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.describe", "arguments":{"collection":"data_people"} })], vec![json!({"type":"token","text":"You could run a group query."})], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on data_people | group country | sort count desc | limit 20"} })], vec![json!({"type":"token","text":"Summary from actual evidence"})], ], llm_requests: Vec::new(), }; let mut events = Vec::new(); service.run(&input, &mut invoker, |event| { events.push(event); true }).unwrap(); assert_eq!(invoker.llm_requests.len(), 4); assert_eq!(invoker.llm_requests[1].str_field("toolChoice"), Some("required")); assert_eq!(invoker.llm_requests[2].str_field("toolChoice"), Some("required")); let retry_messages = invoker.llm_requests[2].array_field("messages").unwrap(); assert_eq!(retry_messages.last().and_then(|message| message.get("role")).and_then(JsonValue::as_str), Some("user")); assert_eq!(retry_messages.last().and_then(|message| message.get("content")).and_then(JsonValue::as_str), Some(REQUIRED_DATABASE_QUERY_RETRY_DIRECTIVE)); assert!(!events.iter().any(|event| event.str_field("type") == Some("message") && event.str_field("content") == Some("You could run a group query."))); assert!(events.iter().any(|event| event.str_field("type") == Some("message") && event.str_field("content") == Some("Summary from actual evidence"))); }
    #[test] fn observed_fields_reports_types_coverage_and_nested_paths() { let documents = vec![ json!({ "_id":"1", "name":"Ada", "active":true, "score":12, "address":{"country":"FR"} }), json!({ "_id":"2", "name":"Grace", "score":null, "address":{"country":"US"} }), ]; let refs = documents.iter().collect::<Vec<_>>(); let (fields, system_fields) = observed_fields(&refs); assert_eq!(system_fields, vec!["_id".to_owned()]); let name = fields .iter() .find(|field| field.str_field("path") == Some("name")) .expect("name field"); assert_eq!(name.get("coverage").and_then(JsonValue::as_f64), Some(1.0)); assert_eq!(name["types"], json!(["string"])); let active = fields .iter() .find(|field| field.str_field("path") == Some("active")) .expect("active field"); assert_eq!(active.get("coverage").and_then(JsonValue::as_f64), Some(0.5)); assert!(fields.iter().any(|field| { field.str_field("path") == Some("address.country") })); let score = fields .iter() .find(|field| field.str_field("path") == Some("score")) .expect("score field"); assert_eq!(score["types"], json!(["null", "number"])); }
    #[test] fn database_describe_uses_count_and_streaming_observation() { let mut invoker = FakeInvoker::default(); let value = invoke_agent_tool( &mut invoker, "database.describe", json!({"collection":"data_people"}), ) .unwrap(); assert_eq!(invoker.calls, vec!["query.execute", "query.execute"]); assert_eq!(value.str_field("collection"), Some("data_people")); assert_eq!(value.u64_field("documents"), Some(2)); assert_eq!(value.u64_field("observedDocuments"), Some(2)); assert_eq!(value.u64_field("observationLimit"), Some(128)); assert_eq!(value.str_field("observationStrategy"), Some("streaming-prefix")); assert_eq!(value.str_field("schemaKind"), Some("observed")); let fields = value.array_field("fields").unwrap(); assert!(fields.iter().any(|field| { field.str_field("path") == Some("country") })); }
    #[test] fn database_analysis_can_issue_multiple_queries_before_answering() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | count"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on people | group country | sort count desc | limit 20"} })], vec![json!({"type":"token","text":"There are people in several countries."})], ], llm_requests: Vec::new(), }; let stats = service.run(&request(), &mut invoker, |_| true).unwrap(); assert_eq!(stats.tool_calls, 2); assert_eq!(stats.llm_calls, 3); assert_eq!( invoker.calls, vec![ "llm.generate", "query.execute", "llm.generate", "query.execute", "llm.generate", ] ); for request in &invoker.llm_requests[1..] { assert_eq!( request.str_field("toolChoice"), Some("auto") ); let names = request.array_field("tools") .expect("database tools") .iter() .filter_map(|tool| tool.str_field("name")) .collect::<Vec<_>>(); assert_eq!(names, vec!["database.describe", "database.query"]); } }
    #[test] fn database_query_limit_forces_a_tool_free_synthesis_turn() { let service = AgentService { config: AgentConfig { max_database_queries: 2, ..AgentConfig::default() }, }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | count"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on people | group country"} })], vec![json!({"type":"token","text":"Summary"})], ], llm_requests: Vec::new(), }; service.run(&request(), &mut invoker, |_| true).unwrap(); let final_request = invoker.llm_requests.last().expect("final request"); assert_eq!( final_request.str_field("toolChoice"), Some("none") ); assert_eq!( final_request.array_field("tools") .map(Vec::len), Some(0) ); assert_eq!( final_request.u64_field("maxTokens"), Some(u64::from(DEFAULT_SYNTHESIS_MAX_TOKENS)) ); }
    #[test] fn database_analysis_compacts_older_tool_pairs_into_evidence() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | group country | sort count desc | limit 20"} })], vec![json!({ "type":"toolCall", "id":"call-2", "name":"database.query", "arguments":{"query":"on people | group city | sort count desc | limit 20"} })], vec![json!({"type":"token","text":"Summary"})], ], llm_requests: Vec::new(), }; service.run(&request(), &mut invoker, |_| true).unwrap(); let third = &invoker.llm_requests[2]; let messages = third.array_field("messages") .expect("messages"); let system = messages .first() .and_then(|message| message.get("content")) .and_then(JsonValue::as_str) .expect("system content"); assert!(system.contains(DATABASE_EVIDENCE_PREFIX.trim())); assert!(system.contains("group country")); assert!(!system.contains("call-1")); let tool_messages = messages .iter() .filter(|message| message.str_field("role") == Some("tool")) .count(); assert_eq!(tool_messages, 1); let latest_tool = messages .iter() .rev() .find(|message| message.str_field("role") == Some("tool")) .expect("latest tool"); assert_eq!( latest_tool.str_field("toolCallId"), Some("call-2") ); }
    ...
    #[test] fn explicit_collection_listing_requires_a_tool_on_first_turn() { let service = AgentService { config: AgentConfig::default(), }; let mut input = request(); input.messages[0].content = "Montre moi les collections disponibles".to_owned(); let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.collections", "arguments":{} })], vec![json!({"type":"token","text":"people"})], ], llm_requests: Vec::new(), }; service.run(&input, &mut invoker, |_| true).unwrap(); assert_eq!( invoker.llm_requests[0].str_field("toolChoice"), Some("required") ); assert_eq!( invoker.llm_requests[0].array_field("tools") .map(Vec::len), Some(1) ); assert_eq!( invoker.llm_requests[0].array_field("tools") .and_then(|tools| tools.first()) .and_then(|tool| tool.get("name")) .and_then(JsonValue::as_str), Some("database.collections") ); assert_eq!( invoker.llm_requests[1].str_field("toolChoice"), Some("none") ); assert_eq!( invoker.llm_requests[1].array_field("tools") .map(Vec::len), Some(0) ); }
    #[test] fn general_request_keeps_tool_choice_auto() { let service = AgentService { config: AgentConfig::default(), }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![vec![json!({"type":"token","text":"Out of scope"})]], llm_requests: Vec::new(), }; service.run(&request(), &mut invoker, |_| true).unwrap(); assert_eq!( invoker.llm_requests[0].str_field("toolChoice"), Some("auto") ); }
    ...
    #[test] fn default_synthesis_budget_is_larger_than_planning_budget() { let service = AgentService { config: AgentConfig::default() }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | count"} })], vec![json!({"type":"token","text":"Summary"})], ], llm_requests: Vec::new(), }; let mut req = request(); req.max_tokens = None; let mut cfg = service.config; cfg.max_database_queries = 1; let service = AgentService { config: cfg }; service.run(&req, &mut invoker, |_| true).unwrap(); let final_request = invoker.llm_requests.last().expect("final request"); assert_eq!( final_request.u64_field("maxTokens"), Some(u64::from(DEFAULT_SYNTHESIS_MAX_TOKENS)) ); }
    #[test] fn explicit_max_tokens_caps_synthesis_too() { let mut req = request(); req.max_tokens = Some(333); let service = AgentService { config: AgentConfig { max_database_queries: 1, ..AgentConfig::default() } }; let mut invoker = FakeInvoker { calls: Vec::new(), llm_outputs: vec![ vec![json!({ "type":"toolCall", "id":"call-1", "name":"database.query", "arguments":{"query":"on people | count"} })], vec![json!({"type":"token","text":"Summary"})], ], llm_requests: Vec::new(), }; service.run(&req, &mut invoker, |_| true).unwrap(); let final_request = invoker.llm_requests.last().expect("final request"); assert_eq!( final_request.u64_field("maxTokens"), Some(333) ); }
