//! OpenGlacier daemon runtime.
#![cfg_attr(rustfmt, rustfmt_skip)]
#![allow(unused)]

use std::{
    env, thread, fs, error::Error, fmt::{self, Display, Formatter}, io::{self, BufReader, Read, Write},
    net::{TcpListener, TcpStream}, path::{Path, PathBuf}, process::{Command, Stdio}, collections::{HashMap, HashSet}, sync::{ atomic::{AtomicBool, AtomicU64, Ordering}, mpsc, Arc, Condvar, Mutex, OnceLock },
...
#[cfg(feature = "events")]
use crate::operation::EventsSubscribeInput;
#[cfg(feature = "data-import")]
use crate::operation::DataWorkerRunInput;
#[cfg(feature = "llm")]
use crate::service::llm::{LlmGenerationEvent, LlmService};
#[cfg(feature = "agent")]
use crate::service::agent::{AgentCapabilityInvoker, AgentCapabilityResponse, AgentError, AgentService};
...
fn bootstrap_fabric_defaults_if_needed( engine: &Engine, credential: &crate::access::identity_file::IdentityCredential, published: ServiceCapabilities, ) -> Result<(), DaemonError> {
    let query = format!(
        ...
        match operation {
            ...
            "query.execute" => {
                ...
                let pipeline = parse_pipeline(query)
                    .map_err(|error| AgentError::capability(operation, error.to_string()))?;
                let plan = crate::query::Planner::new().plan(&pipeline)
                    .map_err(|error| AgentError::capability(operation, error.to_string()))?;
...
fn handle_file_operation( mut writer: &mut TcpStream, reader: &mut BufReader<TcpStream>, settings: &ConnectionSettings, engine: Option<&Engine>, authentication: &ConnectionAuth, delegation: Option<&GatewayDelegation>, operation: RoutedOperation, ) -> Result<bool, ConnectionError> {
    ...
    match operation {
                            ...
                            if existing.as_ref().is_some_and(|entry|entry.kind!=crate::files::FileKind::File){
                            ...
                            let folders = entries.into_iter()
                                .filter(|entry| entry.kind == crate::files::FileKind::Directory)
                            ...
                            match file_store.move_entry(&entry.remote_id,parent_remote.as_deref(),&name){
                                Ok(stored)=>{
                                    ...
                                    match replace_file_entry(engine,id,&updated){
                                        Ok(json)=>{
                                            if updated.kind == crate::files::FileKind::Directory {
                            ...
                            let file_store=or_reject!(scoped_native_file_store(settings,id,&place_id,&instance_id));
                            let entry=or_reject!(load_file_entry(engine,id,&place_id,&instance_id,&file_id));
                            if entry.kind==crate::files::FileKind::Directory {
                            ...
                            match file_store.copy(&entry.remote_id,parent_remote.as_deref(),&name){
                                Ok(stored)=>{
                                    ...
                                    match persist_file_entry(engine,id,&copied){
                                        Ok(json)=>{
                                            if copied.kind == crate::files::FileKind::Directory {
...
fn write_event( connection_id: u64, writer: &mut TcpStream, event: &crate::CoreEvent, ) -> Result<(), ConnectionError> {
...
fn try_write_streaming_request( engine: &Engine, writer: &mut TcpStream, request: &QueryRequest, ) -> Result<bool, ConnectionError> {
    ...
    let terminal_count = planned
        ...
        .operators()
        .last()
        .and_then(crate::query::PhysicalOperator::count_alias)
        .is_some();
    ...
    if !memory_streaming && !governed_blocking {
        if matches!(
            ...
            crate::query::MemoryExecutionMode::GovernedBlocking
        ) {
    ...
    let mut write_document = |id: &crate::storage::DocumentId, document: &Document| {
        ...
        .map_err(|error| {
            crate::engine::EngineError::execution(crate::query::ExecutionError::evaluation(
                error.to_string(),
            ))
        })?;
...
fn file_version_json( version_id:&str,file:&FileEntry,remote_id:&str,metadata:&crate::files::FileMetadata,created_at:u64 )->JsonValue{
    serde_json::json!({
...
fn json_to_file_entry(value:&JsonValue)->Result<FileEntry,&'static str>{
    ...
    let kind=match string("kind").as_deref(){Some("file")=>crate::files::FileKind::File,Some("directory")=>crate::files::FileKind::Directory,_=>return Err("invalid file kind")};
    Ok(FileEntry{
...
fn sync_moved_children(engine:&Engine,id:RequestId,store:&dyn FileStore,place_id:&str,instance_id:&str,parent:&FileEntry)->Result<(),QueryResponse>{
    ...
    for mut json in persisted {
        ...
        if child.kind==crate::files::FileKind::Directory{
...
fn persist_copied_children(engine:&Engine,id:RequestId,store:&dyn FileStore,place_id:&str,instance_id:&str,parent:&FileEntry)->Result<(),QueryResponse>{
    ...
    for stored in remote {
        ...
        if child.kind==crate::files::FileKind::Directory{persist_copied_children(engine,id,store,place_id,instance_id,&child)?;}
    }
    Ok(())
}
...
fn require_identity<'a>( writer: &mut TcpStream, principal: &'a crate::access::auth::Principal, id: RequestId, message: &str, ) -> Result<Option<&'a str>, ConnectionError> {
...
fn ensure_routed_static_authorized( writer: &mut TcpStream, settings: &ConnectionSettings, engine: Option<&Engine>, principal: &crate::access::auth::Principal, operation: &RoutedOperation, ) -> Result<bool, ConnectionError> {
...
fn ensure_operation_resource_authorized( writer: &mut TcpStream, settings: &ConnectionSettings, engine: &Engine, principal: &crate::access::auth::Principal, id: RequestId, operation: OperationKind, resource: &str, ) -> Result<bool, ConnectionError> {
...
fn ensure_authorized( writer: &mut TcpStream, settings: &ConnectionSettings, engine: &Engine, principal: &crate::access::auth::Principal, id: RequestId, action: AuthorizationAction, resource: &str, ) -> Result<bool, ConnectionError> {
...
fn authorize_connection( engine: &Engine, principal: &crate::access::auth::Principal, action: AuthorizationAction, resource: &str, ) -> bool {
...
fn write_authorization_denied( writer: &mut TcpStream, id: RequestId, principal: &crate::access::auth::Principal, action: AuthorizationAction, resource: &str, ) -> Result<(), ConnectionError> {
    let message = match principal {
        crate::access::auth::Principal::Anonymous => format!(
            "authentication is required for {} on {:?}",
            action.as_str(),
            resource
        ),
        crate::access::auth::Principal::Identity { identity_id, .. } => format!(
...
#[inline]
fn execution_row_to_json( row: &crate::query::ExecutionRow, plan: &crate::query::PhysicalPlan, ) -> JsonValue {
    let mut value = document_to_json(row.document());
    if exposes_document_id(plan)
        && !matches!(row.origin(), crate::query::ExecutionRowOrigin::Synthetic)
    {
...
fn exposes_document_id(plan: &crate::query::PhysicalPlan) -> bool {
    plan.operators().iter().all(|operator| {
        matches!(
            operator,
            crate::query::PhysicalOperator::Filter { .. }
                | crate::query::PhysicalOperator::Union { .. }
                | crate::query::PhysicalOperator::Limit { .. }
                | crate::query::PhysicalOperator::Skip { .. }
                | crate::query::PhysicalOperator::Sort { .. }
                | crate::query::PhysicalOperator::Distinct { .. }
        )
    })
}
...
struct BorrowedDocumentId<'a>(&'a crate::storage::DocumentId);
        ...
        S: Serializer,
    {
        match self.0 {
            ...
            Value::Number(number) => match number {
                Number::Signed(value) => {
                    crate::protocol::serialize_js_safe_i64(value, serializer)
                }
                Number::Unsigned(value) => {
                    crate::protocol::serialize_js_safe_u64(value, serializer)
                }
...
fn write_stream_response( writer: &mut TcpStream, response: &StreamResponse, ) -> Result<(), ConnectionError> {
    let encoded =
        crate::protocol::encode_stream_response(response).map_err(ConnectionError::Encode)?;
    writer.write_all(&encoded).map_err(ConnectionError::Write)
}
...
fn file_sync_mutable_ids(entries: &[FileEntry], selection: &crate::files::FileSyncSelection) -> HashSet<String> {
...
fn file_sync_reconcile_local_mutations( source: &mut FileSyncSource<'_>, root: &Path, selection: &crate::files::FileSyncSelection, base: &Path, primary_files: bool, tree: &[FileEntry], paths: &HashMap<String, PathBuf>, index: &FileSyncIndex, ) -> Result<FileSyncMutationSummary, String> {
...
fn file_sync_selected_ids(entries: &[FileEntry], selection: &crate::files::FileSyncSelection) -> Result<HashSet<String>, String> {
    if selection.mode == FileSyncSelectionMode::All {
    ...
    for folder_id in &selection.folder_ids {
        ...
        if folder.kind != crate::files::FileKind::Directory {
...
fn file_sync_relative_paths( entries: &[FileEntry], included: &HashSet<String>, base: &Path, primary_files: bool, claimed: &mut HashMap<String, String>, ) -> HashMap<String, PathBuf> {
    ...
    for entry in ordered {
        ...
        let fallback = if entry.kind == crate::files::FileKind::Directory { "Folder" } else { "File" };
    ...
    if let Some(previous) = existing.as_ref() {
        if previous.materialized_path != relative_path {
            ...
            if old_path.exists() && !destination.exists() && file_sync_local_matches(&old_path, previous) {
                ...
                if logical.kind == crate::files::FileKind::Directory {
    ...
    for place_id in &sync_places {
        ...
        for instance_id in instances.keys() {
            if !effective_selections.iter().any(|selection| selection.place_id.as_str() == place_id.as_str() && selection.app_instance_id.as_str() == instance_id.as_str()) {
                effective_selections.push(crate::files::FileSyncSelection {
                    place_id: place_id.clone(),
...
fn run_upstream_event_relay_session( settings: &ConnectionSettings, event_engine: &EventEngine, ) -> Result<(), String> {
    ...
    loop {
        ...
        if let Ok(event) = serde_json::from_value::<crate::CoreEvent>(message) {
...
enum ConnectionError {
    ...
    Read(io::Error),
    Write(io::Error),
    Encode(crate::protocol::ProtocolError),
...
pub enum DaemonError {
    ...
    BootstrapIdentity(crate::access::identity_file::IdentityFileError),
    NodeIdentity(crate::access::identity_file::IdentityFileError),
    NodeIdentityMissing,
...
mod tests {
    ...
    #[test] fn preserves_stage_spans_from_official_ast() { let source = "from users | where active == true"; let pipeline = parse_pipeline(source).expect("pipeline parses"); let stage = &pipeline.stages()[0]; assert_eq!(stage.span(), crate::query::Span::new(11, source.len())); }
